import { paginationResults } from './utils'
  ...
  Mutation: {
    ...
    bookTrips: async (_: any, { launchIds }: any, { dataSources }: any) => {
      ...
      return {
        ...
        message: results.length === launchIds.length ? 'trips booked successfully' : `the following launches couldn't be booked: ${launchIds.filter((id: any) => !results.includes(id))}`,
        launches,
      }
    },
